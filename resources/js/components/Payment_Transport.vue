                        ...
                        <div class="tour_detail_right_sidebar">
                            <div class="tour_details_right_boxed">
                                <div class="tour_details_right_box_heading">
                                    <h3>{{ oDataProdukPackage.TransportationName }}</h3>
                                </div>
                                <div class="tour_package_details_bar_list">
                                    <h5>Package details</h5>
                                    <p v-html="oDataProdukPackage.TransportationDescription"></p>
                                </div>
                                <div class="tour_package_details_bar_price">
                                    <h5>Price</h5>
                                    <div class="tour_package_bar_price">
                                        <h6 v-if="oDataProdukPackage.PackagePriceDiscount > 0">
                                            <del> {{ formatdefaultNumber(oDataProdukPackage.PackagePrice) }}</del>
                                        </h6>
                                        <h3>
                                             {{ formatdefaultNumber(oDataProdukPackage.PackagePriceDiscount > 0 ? oDataProdukPackage.PackagePriceDiscount : oDataProdukPackage.PackagePrice) }}
                                            <sub>for {{ oDataProduk.TransportationCapacity }}</sub>
                                        </h3>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="tour_detail_right_sidebar">
                        <div class="tour_details_right_boxed">
                            ...
                            <div class="tour_booking_amount_area">
                                <ul>
                                    <li>Adult Price x {{ adult }} <span>{{ formatdefaultNumber(oDataProdukPackage.PackagePrice) }}</span></li>
                                </ul>
                                <div class="tour_bokking_subtotal_area">
                                    <h6>Subtotal <span>{{ formatdefaultNumber(subtotal) }}</span></h6>   
                                </div>
                                <ul>
                                    <li>
                                        Discount ({{ discountPercent }}%)
                                        <span>
                                            -{{ discountPercent > 0 ? formatdefaultNumber(
                                                (oDataProdukPackage.PackagePriceDiscount)
                                            ) : 0 }}
                                        </span>
                                    </li>
                                </ul>
                                <div class="total_subtotal_booking">
                                    <h6>Total Amount <span>{{ formatdefaultNumber(totalAmount) }}</span></h6>
                                </div>
                        ...
                        <div class="tour_details_right_boxed">
                            ...
                            <div class="edit_date_form">
                                <div class="form-group">
                                    ...
                                    <input type="date" id="dates" class="form-control" :value="bookingData.BookingDate" readonly>
                                </div>
                            </div>
                            <div class="tour_package_details_bar_list">
                                <h5>Tourist</h5>
...
export default {
    ...
    mounted() {
        console.log(this.oDataProdukPackage);
    ...
    computed: {
        discountPercent() {
            const originalPrice = this.oDataProdukPackage.PackagePrice;
            const discountPrice = this.oDataProdukPackage.PackagePriceDiscount;
        ...
        // Hitung subtotal
        subtotal() {
            const price = this.oDataProdukPackage.PackagePrice;
        ...
        discountAmount() {
            return this.oDataProdukPackage.PackagePriceDiscount > 0 ?(this.oDataProdukPackage.PackagePrice - this.oDataProdukPackage.PackagePriceDiscount)  : 0;
        },
        // Hitung pajak
        taxAmount() {
