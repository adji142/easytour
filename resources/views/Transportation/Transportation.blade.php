						...
						<div class="card card-custom gutter-b bg-transparent shadow-none border-0" >
							<div class="card-header align-items-center  border-bottom-dark px-0">
								<div class="card-title mb-0">
									<h3 class="card-label mb-0 font-weight-bold text-body">Transportation 
									</h3>
								</div>
							    <div class="icons d-flex">
									<a href="{{ url('transportation/form/0') }}" class="btn btn-outline-primary rounded-pill font-weight-bold me-1 mb-1">Tambah Data</a>
									<div class="icons d-flex">
								</div>
								</div>
							</div>
				...
				<div class="row">
					<div class="col-12  px-4">
						<div class="card card-custom gutter-b bg-white border-0" >
							...
							<div class="card-body" >
								<form action="{{ route('transportation') }}">
									<div class="row">
										<div class="col-md-3">
									...
									<table id="orderTable" class="display" style="width:100%">
										...
										<tbody>
											@if (count($transportation) > 0)
												...
												<tr>
													<td>{{ $v['TransportationName'] }}</td>
													<td>{{ $v['TransportationType'] }}</td>
													<td>{{ $v['TransportationCapacity']  }}</td>
                                                    <td>{!! $v['TransportationDescription'] !!}</td>
													<td> <div class="{{ $v['Status'] == 'Y' ? 'mr-0 text-success' : 'mr-0 text-danger' }} ">{{ $v['StatusDesc'] }}</div> </td>
													<td>
														<div class="card-toolbar text-end">
															...
															<div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdowneditButton1"  style="position: absolute; transform: translate3d(1001px, 111px, 0px); top: 0px; left: 0px; will-change: transform;">
																<a class="dropdown-item" href="{{ url('transportation/form/' . $v['id']) }}">Edit</a>
															</div>
														</div>
													</td>
												</tr>
